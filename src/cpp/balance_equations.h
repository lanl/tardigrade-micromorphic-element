...
namespace balance_equations{
    ...
    void compute_internal_force(const double (&dtestdx)[3], const Vector_9 &cauchy, double (&fint)[3]);
    
    void compute_internal_force(const int &i, const double (&dtest_fxndx)[3], const Vector_9 &cauchy, double &fint_i);

    void compute_body_force(const double &test_fxn, const double &density, const double (&b)[3], double (&fb)[3]);
    
    void compute_body_force(const int &i, const double &test_fxn, const double &density, const double (&b)[3], double &fb);

    void compute_kinematic_force(const double &test_fxn, const double &density, const double (&a)[3], double (&fkin)[3]);
    
    void compute_kinematic_force(const int &i, const double &test_fxn, const double &density, const double (&a)[3], double &fkin);

    //Stresses from the balance of first moment of momentum
    void compute_internal_couple(const double &test_fxn, const double (&dtest_fxndx)[3], const Vector_9 &cauchy, const Vector_9 &s, const Vector_27 &m, double (&cint)[9]);
    
    void compute_internal_couple(const int &i, const int &j, const double &test_fxn, const double (&dtest_fxndx)[3], const Vector_9 &cauchy, const Vector_9 &s, const Vector_27 &m, double &cint_ij);
