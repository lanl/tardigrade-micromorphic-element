...
void explicit_loop(const Eigen::Matrix3d &A, const Eigen::Matrix3d &B, const Eigen::Matrix<double, 9, 9> C, Eigen::Matrix<double, 9, 9> &D){
    /*
    =======================
    |    explicit_loop    |
    =======================
...
int main(){
    ...
    */
    ...
    t0 = Clock::now();
    explicit_loop(A,B,C,D);
    t1 = Clock::now();
    std::cout << "Explicit Loop: " << std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count() << "\n";
    return 1;
}