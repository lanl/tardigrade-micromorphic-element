...
2. Copy the ssh URL from the blue Gitlab "Clone" button on the
   ...
   .. code-block:: bash
      ...
      ssh://git@re-git.lanl.gov:10022/aea/stub-repositories/tardigrade_micromorphic_element.git
...
4. Clone the stub repository using the URL copied in step 2.
   ...
   .. code-block:: bash
      ...
      $ git clone ssh://git@re-git.lanl.gov:10022/aea/stub-repositories/tardigrade_micromorphic_element.git
...
5. Rename the local repository directory for your project.
   ...
   .. code-block:: bash
      ...
      $ ls tardigrade_micromorphic_element -d
      tardigrade_micromorphic_element

      # Rename the stub repo directory after your project
      $ mv tardigrade_micromorphic_element my_project

      # Observe that the stub repo directory no longer exists
      $ ls tardigrade_micromorphic_element -d
      ls: cannot access 'tardigrade_micromorphic_element': No such file or directory
...
3. On the "Create new project" page, follow the link for "Create blank project".
   ...
   .. note::
      ...
      guide much simpler in the future. Contact the ``tardigrade_micromorphic_element`` developers and `AEA
...
6. Remove any issue branches from the ``tardigrade_micromorphic_element`` project on the "Repository" >
...
7. If everything looks correct on Gitlab project, you can clean up your local
   ...
   .. warning::
      ...
      tardigrade_micromorphic_element developers for help.

   .. code-block:: bash

      # Remove the tardigrade_micromorphic_element remote
      $ git remote remove old-origin

      # Ensure that you're on the main branch
      $ git checkout main

      # Remove ALL tardigrade_micromorphic_element branches except main and dev
...
The ``tardigrade_micromorphic_element`` project comes pre-configured to perform continuous integration
...
https://aea.re-pages.lanl.gov/stub-repositories/tardigrade_micromorphic_element. This job should be
...
2. Search for all instances of ``tardigrade_micromorphic_element``. The list of occurrences will look
   ...
   .. code-block:: bash
      ...
      $ grep -ri tardigrade_micromorphic_element . --exclude-dir={build,.git} | wc -l
      57

      # Recursive, case-insensitive search and display
      $ grep -ri tardigrade_micromorphic_element . --exclude-dir={build,.git}
      ...

      # Clean list of files with project name
      $ grep -ri tardigrade_micromorphic_element . --exclude-dir={build,.git} -l
      ./CMakeLists.txt
      ./docs/api.rst
      ./docs/devops.rst
      ./README.md
      ./set_vars.sh
      ./src/cpp/tardigrade_micromorphic_element.cpp
      ./src/cpp/tardigrade_micromorphic_element.h
      ./src/cpp/tests/test_tardigrade_micromorphic_element.cpp
...
3. Search and replace from command line
   ...
   .. code-block:: bash
      ...
      $ sed -i 's/tardigrade_micromorphic_element/my_project/g' $(grep -ri tardigrade_micromorphic_element . --exclude-dir={build,.git} -l)
      $ grep -ri tardigrade_micromorphic_element . --exclude-dir={build,.git} -l
      ./src/cpp/tardigrade_micromorphic_element.h
...
4. Verify no more occurrences of project name ``tardigrade_micromorphic_element``
...
5. Search and replace camelCase project name occurrences, e.g. ``tardigradeMicromorphicElement``.

   .. code-block:: bash

      $ grep -r tardigradeMicromorphicElement . --exclude-dir={build,.git}
      ...
      $ sed -i 's/tardigradeMicromorphicElement/myProject/g' $(grep -r tardigradeMicromorphicElement . --exclude-dir={build,.git} -l)
      $ grep -r tardigradeMicromorphicElement . --exclude-dir={build,.git} -l
      # no stdout to terminal because no files found
...
6. Find files containing the project in their file name
   ...
   .. code-block:: bash
      ...
      $ pwd
      /projects/<moniker>/w13repos/my_project
      $ find . -type d \( -name .git -o -name build \) -prune -false -o -name "*tardigrade_micromorphic_element*"
      ./src/cpp/tardigrade_micromorphic_element.cpp
      ./src/cpp/tardigrade_micromorphic_element.h
      ./src/cpp/tests/test_tardigrade_micromorphic_element.cpp
...
7. Rename files after current project
   ...
   .. code-block:: bash
      ...
      $ rename tardigrade_micromorphic_element myproject $(find . -type d \( -name .git -o -name build \) -prune -false -o -name "*tardigrade_micromorphic_element*")
...
8. Commit and push your changes to your "remote" or "fork" repository
   ...
   .. code-block:: bash
      ...
      $ git commit -a -m "FEAT: replace tardigrade_micromorphic_element with my_project throughout repository"
